...
public class HiddenPowerRNG {
    ...
    public void setHiddenPower(int hp, int atk, int def, int spa, int spd, int spe) {
        this.type = (((hp % 2) + 2 * (atk % 2) + 4 * (def % 2) + 8 * (spe % 2) + 16 * (spa % 2) + 32 * (spd % 2)) * 15
            / 63);
        this.damage = 30 + (((hp >> 1) % 2) + 2 * ((atk >> 1) % 2) + 4 * ((def >> 1) % 2) + 8 * ((spe >> 1) % 2)
    ...
    public static String hp_tostr[] = { "Fighting", "Flying", "Poison", "Ground", "Rock", "Bug", "Ghost", "Steel",
