...
public class Pokemon {
    ...
    /**
     * Constructor. Level and Stats are calculated from IV, EV, level. Moves aren't
     * added in the constructor, they are added later.
     * 
     * @param n
     * @param level
     * @param IV
     ...
     * @param d
     */
    private Pokemon(int n, int level, int exp, StatPack stat, StatPack IV, StatPack EV, Nature nature, Ability ability,
        PokemonItem heldItem, boolean pokerus, boolean outsider, Move a, Move b, Move c, Move d) {
        this.n = n;
        this.baseData = new PokemonData(n);
        this.level = level;
        this.experience = exp;
    ...
    private short calculateStat(int i) {
        int aux;
        if (i == 0) {
            aux = ((this.IV.hp + (2 * this.getBaseData().baseStat.hp) + (this.EV.hp / 4) + 100) * this.level) / 100
                + 10;
            return (short) aux;
        } else {
            double aux_d = (double) (((this.IV.getStat(i) + (2 * this.getBaseData().baseStat.getStat(i))
                + (this.EV.getStat(i) / 4)) * this.level) / 100 + 5) * this.nature.getNatureBoost(i);
            aux = (int) aux_d;
    ...
    public void checkEvolution() {
        if (this.heldItem.getID() == PokemonItem.EVERSTONE)
            return;
        ...
        for (int i = 0; i < _NUMROWS; i++) {
            ...
            if (this.baseData.getID() == Integer.parseInt(data[0]) && data[2].equals("Level Up")
    ...
    public Pokemon getCopy() {
        Pokemon pkm = new Pokemon(this.n, this.level, this.experience, this.stat.getCopy(), this.IV.getCopy(),
            this.EV.getCopy(), this.nature.getCopy(), this.ability.getCopy(), this.heldItem.getCopy(), this.pokerus,
            this.outsider, this.move[0].getCopy(), this.move[1].getCopy(), this.move[2].getCopy(),
            this.move[3].getCopy());
        return pkm;
    }

    public PokemonType getHiddenPowerType() {
        int type = (((IV.hp % 2) + 2 * (IV.atk % 2) + 4 * (IV.def % 2) + 8 * (IV.spe % 2) + 16 * (IV.spa % 2)
            + 32 * (IV.spd % 2)) * 15 / 63);
        return new PokemonType(type);
    }

    public int getHiddenPowerDamage() {
        return 30 + (((IV.hp >> 1) % 2) + 2 * ((IV.atk >> 1) % 2) + 4 * ((IV.def >> 1) % 2) + 8 * ((IV.spe >> 1) % 2)
            + 16 * ((IV.spa >> 1) % 2) + 32 * ((IV.spd >> 1) % 2)) * 40 / 63;
    }
