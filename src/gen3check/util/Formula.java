...
public class Formula {
     ...
     */
    public static int calculateExperience(Pokemon pkmUsed, Pokemon pkmDefeated, int usedPokemonAmount,
        boolean isTrainer) {
        if (usedPokemonAmount == 0)
            return 0;
     ...
     */
    public static int calculateDamage(int level, int basepower, int atk, int def, float mod1, int crit, float mod2,
        int rand, float stab, float supereffect1, float supereffect2, float mod3) {
        int aux = level * 2 / 5;
        aux = aux + 2;
        aux *= basepower * atk;
        aux = aux / 50;
