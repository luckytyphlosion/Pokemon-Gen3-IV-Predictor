            ...
            private static final long serialVersionUID = 1L;
            {
                c.addPokemonListContainerObserver(new PokemonListContainerObserverHelper() {
                    ...
                    public void pokemonListChange(final List<PokemonFoundData> pokemonList) {
                        ...
                        // avoiding to update the table multiple times in a few miliseconds (else we can
                        // get errors)
                                ...
                                for (int i = 0; i < pokemonList.size(); i++) {
                                    ...
                                    model.addRow(new Object[] { frame_aux,
                                        Integer.toString(frame_aux / 3600) + ":"
                                            + String.format("%02d", (frame_aux / 60) % 60) + "."
                                            + String.format("%02d", (frame_aux % 60) * 100 / 60),
                                        pokemon_aux.nature, pokemon_aux.hp, pokemon_aux.atk, pokemon_aux.def,
                                        pokemon_aux.spa, pokemon_aux.spd, pokemon_aux.spe, gender_str,
                                        HiddenPowerRNG.hp_tostr[hpow.type], hpow.damage });
                                }
                                model.fireTableDataChanged();
        ...
        for (int i = 0; i < pokemonList.size(); i++) {
            ...
            model.addRow(new Object[] { frame_aux,
                Integer.toString(frame_aux / 3600) + ":" + String.format("%02d", (frame_aux / 60) % 60) + "."
                    + String.format("%02d", (frame_aux % 60) * 100 / 60),
                pokemon_aux.nature, pokemon_aux.hp, pokemon_aux.atk, pokemon_aux.def, pokemon_aux.spa, pokemon_aux.spd,
                pokemon_aux.spe, gender_str, HiddenPowerRNG.hp_tostr[hpow.type], hpow.damage });
        }
        model.fireTableDataChanged();
        if (pokemonList.size() > 0) {
